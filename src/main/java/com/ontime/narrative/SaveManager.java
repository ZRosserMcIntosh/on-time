...
/**
 * Handles saving and loading game progress for On Time.
 */
public class SaveManager {
        ...
        if (Files.exists(savePath)) {
            try {
                ...
                System.out.println("[Save] Loaded save — Chapter: " + state.getCurrentChapterId()
                        ...
                        + ", Est. playtime: " + String.format("%.1f", state.getEstimatedPlaytimeHours()) + "h");
                return state;
            } catch (Exception e) {
    ...
    public static void save(StoryState state) {
        try {
            ...
            System.out.println("[Save] Progress saved. Est. playtime: "
                    + String.format("%.1f", state.getEstimatedPlaytimeHours()) + "h");
        } catch (Exception e) {
